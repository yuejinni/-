...
IDLE_FRAME_TIMEOUT = 0.2
CONTIGUOUS_HEADER = re.compile(rb"\d{4}#")

Frame = Tuple[str, bytes]


def extract_frames(buffer: bytearray,
                   flush_contiguous: bool = False) -> Tuple[List[Frame], List[bytes]]:
    ...
    while buffer:
        ...
        if crlf_at < 0:
            # 平台未配置边界符时，会连续发送：
            # 0121#NoRead0122#230921C029015...
            # 下一条四位小车号头即为上一条的结束边界。
            headers = list(CONTIGUOUS_HEADER.finditer(buffer))
            if headers:
                first = headers[0]
                if first.start() > 0:
                    dropped.append(bytes(buffer[:first.start()]))
                    del buffer[:first.start()]
                    continue
                if len(headers) >= 2:
                    end = headers[1].start()
                    frames.append(("CONTIGUOUS", bytes(buffer[:end])))
                    del buffer[:end]
                    continue
                if flush_contiguous and first.start() == 0:
                    frames.append(("CONTIGUOUS", bytes(buffer)))
                    buffer.clear()
            break
        ...
        while True:
            try:
                data = await asyncio.wait_for(
                    reader.read(4096),
                    timeout=IDLE_FRAME_TIMEOUT,
                )
            except asyncio.TimeoutError:
                frames, dropped = extract_frames(
                    buffer, flush_contiguous=True
                )
